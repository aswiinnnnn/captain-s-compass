...
import { Download, Zap, ChevronRight, ArrowUp, ArrowDown, Minus, TrendingUp, AlertTriangle, Clock, Users, Anchor, Timer, TriangleAlert, DollarSign, Sparkles, Bell, Settings, Search } from 'lucide-react';
...
const BiddingDetail = () => {
  ...
  const handleWheel = useCallback((e: React.WheelEvent) => {
    e.preventDefault();
    if (e.deltaY < 0) setViewCount(v => Math.max(10, v - 3));
    else setViewCount(v => Math.min(dailyData.length, v + 3));
  }, [dailyData.length]);
  ...
  const CustomDot = (props: any) => {
    ...
    return <circle cx={cx} cy={cy} r={5} fill="hsl(224, 76%, 48%)" stroke="white" strokeWidth={2} />;
  };
    ...
    <div className="h-screen bg-background flex flex-col overflow-hidden">
      ...
      <header className="h-14 border-b border-border flex items-center justify-between px-6 bg-card shrink-0">
        <div className="flex items-center gap-6">
          ...
          <nav className="hidden md:flex items-center gap-1">
            ...
            <NavTab active onClick={() => navigate('/dashboard')}>Bidding Hub</NavTab>
      ...
      <div className="border-b border-border px-6 py-2 bg-card shrink-0">
        <div className="flex items-center gap-2 text-xs">
          <button onClick={() => navigate('/dashboard')} className="text-primary hover:underline font-medium">Bidding Hub</button>
          <ChevronRight className="w-3 h-3 text-muted-foreground" />
          <span className="text-muted-foreground">{canal.name} - Transit SC-4402</span>
        </div>
        <div className="flex items-center justify-between mt-1">
          <div>
            <h1 className="text-lg font-bold text-foreground">{canal.name} Transit Bidding</h1>
            <p className="text-[10px] text-muted-foreground">Vessel: MV Northern Star | ETA: Oct 24, 08:00 UTC</p>
          </div>
          <div className="flex items-center gap-2">
      ...
      {/* Full-width scrollable analytics */}
      <div className="flex-1 overflow-y-auto p-5 space-y-4 min-h-0">
        ...
        <div className="glass-panel rounded-xl p-4">
          ...
          <div className="flex items-center gap-4 mb-2 text-[10px]">
            ...
            <span className="text-muted-foreground ml-auto">Scroll to zoom · Drag to pan</span>
          </div>
          <div
            ref={chartRef}
              ...
              <div className="relative ml-4">
                ...
                <div className="relative flex gap-4 mb-6">
                  ...
                  <div className="pt-0.5">
                    ...
                    <span className="inline-block mt-1.5 text-[9px] font-bold px-2 py-0.5 rounded-full bg-destructive/10 text-destructive">Queue Status: High congestion</span>
                  </div>
                </div>
                <div className="relative flex gap-4 mb-6">
